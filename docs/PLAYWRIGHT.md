...
> **`--browser chromium` を必ず付ける（重要）**
> 付けないと Playwright MCP は既定で **Google Chrome 本体**（`/opt/google/chrome/chrome`）を探しに行き、
> 手順 2 で入れる **ブラウザ**とは別物のため `Chromium distribution 'chrome' is not found` で開けません。
> Chrome 本体の導入は `sudo`（管理者権限）が必要なので、手順 2 のブラウザを使う `--browser chromium` 指定が確実です。
> `--browser chromium` を指定すると、MCP は **Playwright がバンドルする Chromium ビルド（Chrome for Testing）** を使います（その実体は手順 2 で入れます）。
> `--` は「これ以降は MCP サーバーへ渡す引数」という区切りで、付けないと `claude mcp add` 自身のオプションと誤解されます。
...
`@playwright/mcp` は **自身のバージョンに対応したビルド（Chrome for Testing）** を要求するため、
**MCP 付属のインストーラで入れるのが確実**です。

```bash
# MCP が要求するブラウザ（Chrome for Testing）を入れる ← これが確実
npx @playwright/mcp@latest install-browser chrome-for-testing

# WSLでは依存ライブラリが不足するのでこのコマンドも実行する
npx playwright install-deps
```

> **なぜ `npx playwright install chromium` ではないのか**
> 素の `npx playwright install chromium` でも動くことはありますが、その Playwright CLI と
> `@playwright/mcp@latest` の **バージョン（ビルド番号）がズレる**と、実体があっても
> `Browser "chrome-for-testing" is not installed` になります（例: CLI 側は `chromium-1223`、
> MCP 側は `chromium-1226` を要求、など）。上の **MCP 付属インストーラ**なら MCP が要求する
> ビルドをピンポイントで入れるため整合が取れます。
> 全ブラウザが必要なら `npx playwright install`。
...
# （venv を有効化済みなら `streamlit run app.py` でも可）
```
...
- **`Chromium distribution 'chrome' is not found at /opt/google/chrome/chrome`** → MCP の登録に `--browser chromium` が付いていない。手順 1 のコマンド（`-- npx @playwright/mcp@latest --browser chromium`）で登録し直し、**Claude Code を再起動**して反映する（MCP 設定の変更は稼働中プロセスには反映されない）。
- **`Browser "chrome-for-testing" is not installed. Run npx @playwright/mcp install-browser chrome-for-testing`** → `@playwright/mcp` が要求するブラウザビルドが未導入。表示どおり **`npx @playwright/mcp@latest install-browser chrome-for-testing`** を実行する。`npx playwright install chromium` 済みでも、CLI と MCP のビルド番号がズレていると出る（手順 2 参照）。
- **ブラウザ起動でエラー** → 手順 2 の `npx @playwright/mcp@latest install-browser chrome-for-testing`（必要なら `install-deps`）を実行。
