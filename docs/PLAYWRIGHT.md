...
> 手順 2 で入れる **chromium** とは別物のため `Chromium distribution 'chrome' is not found` で開けません。
> Chrome 本体の導入は `sudo`（管理者権限）が必要なので、手順 2 の chromium を使う `--browser chromium` 指定が確実です。
