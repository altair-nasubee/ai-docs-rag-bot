...
| 公開先 | Streamlit Community Cloud または Hugging Face Spaces | アプリを Web に公開 | 無料 |
...
- **公開先**: Streamlit Community Cloud（1GB）または **Hugging Face Spaces（16GB・メモリ的に安全）**。どちらも無料・揮発性FS・スリープあり。e5-small はモデル底値 ~930MB のため、メモリ余裕のある HF Spaces を推奨。
...
### 実装状況（再開用メモ・2026-06-23 時点）

- **コードは STEP 1〜5 まで実装済み**（`config.py` / `embedding.py` / `ingest.py` / `rag.py` / `updater.py` / `app.py`、全モジュール構文OK）。STEP 6 以降は未着手。
- **埋め込みを Gemini → ローカル ONNX（fastembed `intfloat/multilingual-e5-small`）に移行**。`embedding.py` は `LocalEmbeddings`（カスタム登録・`query:`/`passage:` プレフィックス・L2正規化・`batch_size` 制御）。`requirements.txt` から `langchain-google-genai` 撤去・`fastembed` 追加。`.env` も更新済み（`GOOGLE_API_KEY` 不要）。
- **チャンク設定**: `CHUNK_SIZE=1500` 文字（e5-small の512トークン枠内）/ `CHUNK_OVERLAP=200`。コーパス全体で**約4,356チャンク**。
- **メモリ対策（重要）**: fastembed 既定 `batch_size=256` は onnxruntime 活性化メモリが ~4GB に急増し低メモリ機（このdev機は WSL2・総RAM 5.8GB）で OOM/スワップthrash する。`EMBED_BATCH_SIZE=8` でピーク ~1.1GB に抑制。
- **`ingest.py` は resumable**（本文ハッシュ一致はスキップ）。ただし**複数回の再開はタクソノミ混在を招く**ため、正準DBは `python ingest.py --reset` の**単一実行**で作る。
- **✅ ブロッカー解消**: ローカル埋め込み化でクォータ・429 が消滅。**基準DB構築完了（150ページ / 約4,356チャンク / 12カテゴリ・L2正規化 norm=1.0・クロスリンガル検索OK）**。
- **解消した保留事項**: 旧「実行時埋め込みのフェイルファスト化（案A/B）」は、埋め込みがローカル（API待ち無し）になったため不要に。
- **次**: STEP6 通し動作確認（`streamlit run app.py`）→ デプロイ（HF Spaces 推奨・`README.md` frontmatter・`data/` を git 同梱）。
...
- [x] `config.py` を作成（`os.getenv` と `st.secrets` の両対応でキー・モデル名・パスを返す）。throttle 用 `EMBED_RPM_LIMIT` / `EMBED_BATCH_SIZE` も追加。
...
- [x] Chroma を読み込み、**カテゴリ指定は任意**（未指定なら全体検索）の検索を構成（**取得件数 k=4**＝Groq 6,000 TPM に配慮）。質問の埋め込みは `query: ` プレフィックス・384次元・L2正規化で取り込み側と統一。質問ベクトルは1回だけ算出し検索と関連分野推定で再利用。
- [x] 取得文書から**参照カテゴリと出典 URL を抽出して返す**（案4の表示用）。
- [x] 質問ベクトルとカテゴリ代表ベクトルの類似度から**関連分野を推定する関数**を用意（案3、追加 API なし＝`related_categories`）。
- [x] 回答プロンプトを作成（英語ドキュメントの内容のみを根拠に**日本語で回答**・出典 URL を併記）。
...
- [x] サイドバーは補助情報（説明・ステータス・会話クリア）のみに限定。
...
- [ ] ブラウザで実操作: **カテゴリ未選択でも回答できること**・関連分野/参照分野/出典表示・カテゴリ動的生成を目視確認。
...
### フェーズ3: 公開（Hugging Face Spaces 推奨 / Streamlit Community Cloud）

#### STEP 9: デプロイ
- [ ] 公開 GitHub リポジトリを用意（`data/chroma` と `manifest.json` を同梱。`data/llm_cache.db` は `.gitignore` 済み）。
- [ ] **Hugging Face Spaces（16GB・メモリ安全）** か Streamlit Community Cloud（1GB）に `app.py` をデプロイ。HF Spaces 用に `README.md` 先頭の YAML frontmatter（`sdk: streamlit`/`app_file: app.py`/`sdk_version`）を用意済み。
- [ ] Secrets（HF は環境変数 / Streamlit は `st.secrets`）に **`GROQ_API_KEY` のみ**を登録（埋め込みはローカルのためキー不要）。
- [ ] メモリに収まることを確認（埋め込みモデル底値 ~930MB。1GB 環境では際どいため HF Spaces を推奨）。
...
- **【重要】ファイルシステムは揮発性（Community Cloud / HF Spaces とも）**: コールドスタートのたびにコンテナは **git にコミットされた状態へ戻り、実行中に書き込んだ差分（更新後の Chroma / `manifest.json`）は永続しない**。
