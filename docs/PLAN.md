...
| 公開先          | **Hugging Face Spaces**（CPU Basic・16GB・Docker SDK で Streamlit 起動） | アプリを Web に公開                  | 無料                 |
...
- **公開先は Hugging Face Spaces に決定**（CPU Basic・**16GB RAM**・無料。HF は Streamlit 単体SDKを廃止したため **Docker SDK＋`Dockerfile` で Streamlit を起動**）。e5-small はモデル底値 ~930MB のため 1GB の Streamlit Community Cloud では際どく、メモリ余裕のある HF Spaces を採用。HF はシークレットを環境変数として渡すため `config.py` の `os.getenv` がそのまま動く。揮発性FS・48時間スリープあり。
...
> 詳細なステップバイステップ手順は `[docs/DEPLOY.md](DEPLOY.md)` を参照。

- [x] **Docker SDK の Space を作成**（CPU Basic・無料。作成画面で Docker→Streamlit テンプレート）。`README.md` frontmatter（`sdk: docker` / `app_port: 8501`）と `Dockerfile` は用意済み。
