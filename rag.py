...
以下の「参考ドキュメント」を根拠に、ユーザーの質問へ日本語で答えてください。

ルール:
- 参考ドキュメントから読み取れる範囲で、できるだけ役立つ回答をする。直接の記述が無くても、
  文書中の関連情報を統合して説明してよい。
- ただし参考ドキュメントに無い事実は創作しない。本当に関連情報が皆無のときだけ
  「ドキュメントには十分な情報が見つかりませんでした」と述べる。
- コードやコマンドは原文のまま示す。
- 簡潔に、必要なら箇条書きで。
...
def _display_url(url: str) -> str:
    """出典表示用URL。`.md`（生Markdown）を外し、日本語の HTML ページへリンクする。

    例: .../docs/en/interactive-mode.md → .../docs/ja/interactive-mode
    （取り込み元は英語 .md だが、表示は日本語ページにする）。
    """
    if url.endswith(".md"):
        url = url[:-3]
    return url.replace("/docs/en/", "/docs/ja/")
    ...
    def _format_context(
        ...
        for doc in docs:
            ...
            if url and url not in seen_urls:
                seen_urls.add(url)
                sources.append({"title": title, "url": _display_url(url)})
